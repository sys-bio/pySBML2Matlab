...
# - function names
# TODO: add flux names!!!
def replaceStringToken(sbmlRef, currentToken : str, reactionId : str, divideVolumes : bool = True):

    replaceString = ''
    innerString = stringInside(currentToken)        
    localParameterId = reactionId + "_" + innerString;

    if currentToken in sbmlRef.model.getListOfParameterIds ():
       return sbmlRef.parameterMap[currentToken].matlabSymbol
    if currentToken in sbmlRef.model.getListOfBoundarySpecies():
       return sbmlRef.parameterMap[currentToken].matlabSymbol
    if currentToken in sbmlRef.model.getListOfFloatingSpecies():
       return sbmlRef.speciesMap[currentToken].matlabSymbol
    
    return currentToken   
    #if currentToken in sbmlRef.model.getListOfFloatingSpecies ():
        

    # if modelData.parameterDict[innerString] != modelData->globalParamIndexList.end():
    #     {
    #         isBoundarySpecies = False

    #         for ib in range (sbmlModel.getNumBoundarySpecies()):

    #             if (innerString == modelData.species[ib + _currentModel->numFloatingSpecies].id) 
    #             {
    #                 if (divideVolumes):
    #                     replaceStream += "("

    #                 if (_bInlineMode):
    #                    replaceStream += modelData.prametersDict[innerString]
    #                 else:
    #                    replaceStream += "rInfo.g_p" + modelData.parameterDict[innerString]
...
def subsituteConstants(sbmlRef, equation, reactionId, divideVolumes = True):
   ...
   try:
      for tok in lx.tokens():
          if tok.type == "IDENTIFIER":
             ...
             result += replaceStringToken(sbmlRef, currentToken,  reactionId, divideVolumes);
...
# def subConstantsCol(const string &equation, const string &reactionId, divideVolumes = True):
# 		# set up the scanner
